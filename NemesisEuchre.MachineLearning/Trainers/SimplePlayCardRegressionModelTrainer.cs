...
public class SimplePlayCardRegressionModelTrainer(
    MLContext mlContext,
    IDataSplitter dataSplitter,
    IModelPersistenceService persistenceService,
    IOptions<MachineLearningOptions> options,
    ILogger<SimplePlayCardRegressionModelTrainer> logger)
    : RegressionModelTrainerBase<SimplePlayCardTrainingData>(mlContext, dataSplitter, persistenceService, options, logger)
{
    protected override IEstimator<ITransformer> BuildPipeline(IDataView trainingData)
    {
        var featureColumns = FeatureColumnProvider.GetFeatureColumns<SimplePlayCardTrainingData>();
