...
public class AdvancedPlayCardRegressionModelTrainer(
    MLContext mlContext,
    IDataSplitter dataSplitter,
    IModelPersistenceService persistenceService,
    IOptions<MachineLearningOptions> options,
    ILogger<AdvancedPlayCardRegressionModelTrainer> logger)
    : RegressionModelTrainerBase<AdvancedPlayCardTrainingData>(mlContext, dataSplitter, persistenceService, options, logger)
{
    protected override IEstimator<ITransformer> BuildPipeline(IDataView trainingData)
    {
        var featureColumns = FeatureColumnProvider.GetFeatureColumns<AdvancedPlayCardTrainingData>();
