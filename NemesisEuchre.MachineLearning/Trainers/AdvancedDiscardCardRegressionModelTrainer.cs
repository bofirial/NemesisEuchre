...
public class AdvancedDiscardCardRegressionModelTrainer(
    MLContext mlContext,
    IDataSplitter dataSplitter,
    IModelPersistenceService persistenceService,
    IOptions<MachineLearningOptions> options,
    ILogger<AdvancedDiscardCardRegressionModelTrainer> logger)
    : RegressionModelTrainerBase<AdvancedDiscardCardTrainingData>(mlContext, dataSplitter, persistenceService, options, logger)
{
    protected override IEstimator<ITransformer> BuildPipeline(IDataView trainingData)
    {
        var featureColumns = FeatureColumnProvider.GetFeatureColumns<AdvancedDiscardCardTrainingData>();
